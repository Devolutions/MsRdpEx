...
#define SYSMENU_RDP_SYNC_DISPLAY_SETTINGS_ID     7104
#define SYSMENU_RDP_MAXIMIZE_AND_RESIZE_ID       7105
#define SYSMENU_RDP_RANGE_LAST_ID                7106
...
LRESULT CALLBACK Hook_TscShellContainerWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    if (uMsg == WM_GETMINMAXINFO)
    {
        ...
        CopyMemory(&minMaxInfo, pMinMaxInfo, sizeof(MINMAXINFO));
        ...
        switch (wParam)
        {
            ...
            case SYSMENU_RDP_RESIZE_TO_FIT_WINDOW_ID:
                MsRdpEx_LogPrint(DEBUG, "Resize to fit window");
                {
                    ...
                    HWND hOPContainerWnd = GetParent(g_hOPWnd);
                    HWND hUIMainWnd = GetParent(GetParent(hOPContainerWnd));

                    char className[256];
                    int classNameLength = GetClassNameA(hUIMainWnd, className, sizeof(className));
                    MsRdpEx_LogPrint(DEBUG, "UIMainWindow class name: %s", className);
                    ...
                    bool hasHorizontalScrollbar = (GetWindowLong(hUIMainWnd, GWL_STYLE) & WS_HSCROLL) != 0;
                    bool hasVerticalScrollbar = (GetWindowLong(hUIMainWnd, GWL_STYLE) & WS_VSCROLL) != 0;

                    MsRdpEx_LogPrint(DEBUG, "scrollbars: horizontal: %d (%d) vertical: %d (%d)",
                        hasHorizontalScrollbar ?1:0, scrollbarWidth, hasVerticalScrollbar ?1:0, scrollbarHeight);
                    MsRdpEx_LogPrint(DEBUG, "windowWidth: %d windowHeight: %d", windowWidth, windowHeight);
                    MsRdpEx_LogPrint(DEBUG, "clientWidth: %d clientHeight: %d", clientWidth, clientHeight);
                    MsRdpEx_LogPrint(DEBUG, "borderWidth: %d borderHeight: %d", borderWidth, borderHeight);

                    ULONG ulDesktopWidth = clientWidth;
                    ULONG ulDesktopHeight = clientHeight;
                    ...
                    //ShowScrollBar(hUIMainWnd, SB_BOTH, FALSE);
                    //SetWindowPos(hUIMainWnd, NULL, 0, 0, ulDesktopWidth, ulDesktopHeight, 0);
                    //SetWindowPos(hOPContainerWnd, NULL, 0, 0, ulDesktopWidth, ulDesktopHeight, 0);
                    //SetWindowPos(g_hOPWnd, NULL, 0, 0, ulDesktopWidth, ulDesktopHeight, 0);
            ...
            case SYSMENU_RDP_SYNC_DISPLAY_SETTINGS_ID:
                MsRdpEx_LogPrint(DEBUG, "Sync display settings");
                {
                    IUnknown* pUnknown = NULL;
                    IMsRdpClient9* pMsRdpClient9 = NULL;
                    instance->GetRdpClient((LPVOID*)&pUnknown);
                    pUnknown->QueryInterface(IID_IMsRdpClient9, (LPVOID*)&pMsRdpClient9);
                    pMsRdpClient9->SyncSessionDisplaySettings();
                }
                break;
