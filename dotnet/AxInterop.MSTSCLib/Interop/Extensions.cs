...
namespace MSTSCLib
{
    public static class MSTSCLibExtensions
    {
        ...
        public static bool TryGetInterface<T>(this object? instance, out T? result) where T : class
        {
            return ProxyObject.TryPack(instance, out result);
        }

        public static void SetLoadBalanceInfoBytes(this IMsRdpClientAdvancedSettings settings, ReadOnlySpan<byte> value)
        {
            ArgumentNullException.ThrowIfNull(settings);
            settings.LoadBalanceInfo = new BinaryString(value);
        }

        public static ImmutableArray<byte> GetLoadBalanceInfoBytes(this IMsRdpClientAdvancedSettings settings)
        {
            ArgumentNullException.ThrowIfNull(settings);
            return settings.LoadBalanceInfo?.ToImmutableArray() ?? [];
        }
