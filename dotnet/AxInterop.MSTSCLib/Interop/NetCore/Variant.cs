...
namespace MsRdpEx.Interop
{
    ...
    public static unsafe class VariantMarshaller
    {
        public static object? ConvertToManaged(NativeVariant data)
        {
            switch (data.Type)
            {
                ...
                case VariantType.ArrayModifier | VariantType.UInt8:
                    return VariantInterop.GetByteArray(data.Content1);

                case VariantType.ByRefModifier | VariantType.Error: return *(int*)data.Content1;
                case VariantType.ByRefModifier | VariantType.Boolean: return (bool)new VariantBool(*(short*)data.Content1);
                ...
                case VariantType.ByRefModifier | VariantType.Currency: return decimal.FromOACurrency(*(long*)data.Content1);
                case VariantType.ByRefModifier | VariantType.DateTime: return DateTime.FromOADate(*(double*)data.Content1);
        ...
        public static NativeVariant ConvertToUnmanaged(object? data)
        {
            ...
            switch (data)
            {
                ...
                case byte[] content:
                    result.Type = VariantType.ArrayModifier | VariantType.UInt8;
                    result.Content1 = VariantInterop.CreateByteArray(content);
                    break;

                default:
                    object comObject = data is ProxyObject ? ProxyObject.Unpack(data)! : data;
                    if (!ComWrappers.TryGetComInstance(comObject, out nint unknown))
                        throw new NotSupportedException($"Cannot marshal {data.GetType().FullName} as a VARIANT.");

                    result.Type = VariantType.IUnknown;
                    result.Content1 = unknown;
                    break;
            }
            return result;
        }
    ...
    {
        internal static nint CreateByteArray(ReadOnlySpan<byte> value)
        {
            nint array = SafeArrayCreateVector((ushort)VariantType.UInt8, 0, (uint)value.Length);
            if (array == 0)
            ...
            try
            {
                if (!value.IsEmpty)
                {
                    int hr = SafeArrayAccessData(array, out nint data);
                    ...
                    {
                        Marshal.ThrowExceptionForHR(SafeArrayUnaccessData(array));
                    }
                }
        ...
        internal static byte[] GetByteArray(nint array)
        {
            if (array == 0)
                return [];

            int hr = SafeArrayGetVartype(array, out ushort elementType);
            Marshal.ThrowExceptionForHR(hr);
            if (elementType != (ushort)VariantType.UInt8)
                throw new NotSupportedException($"SAFEARRAY element type {elementType} is not supported.");

            hr = SafeArrayGetLBound(array, 1, out int lowerBound);
            Marshal.ThrowExceptionForHR(hr);
            hr = SafeArrayGetUBound(array, 1, out int upperBound);
            Marshal.ThrowExceptionForHR(hr);
            ...
            hr = SafeArrayAccessData(array, out nint data);
            Marshal.ThrowExceptionForHR(hr);
            try
            {
                return new ReadOnlySpan<byte>((void*)data, length).ToArray();
            }
            finally
            {
                Marshal.ThrowExceptionForHR(SafeArrayUnaccessData(array));
        ...
        [LibraryImport("oleaut32", EntryPoint = "SafeArrayCreateVector", SetLastError = false)]
        internal static partial nint SafeArrayCreateVector(ushort variantType, int lowerBound, uint count);

        [LibraryImport("oleaut32", EntryPoint = "SafeArrayDestroy", SetLastError = false)]
        internal static partial int SafeArrayDestroy(nint array);

        [LibraryImport("oleaut32", EntryPoint = "SafeArrayAccessData", SetLastError = false)]
        internal static partial int SafeArrayAccessData(nint array, out nint data);

        [LibraryImport("oleaut32", EntryPoint = "SafeArrayUnaccessData", SetLastError = false)]
        internal static partial int SafeArrayUnaccessData(nint array);

        [LibraryImport("oleaut32", EntryPoint = "SafeArrayGetVartype", SetLastError = false)]
        internal static partial int SafeArrayGetVartype(nint array, out ushort variantType);

        [LibraryImport("oleaut32", EntryPoint = "SafeArrayGetLBound", SetLastError = false)]
        internal static partial int SafeArrayGetLBound(nint array, uint dimension, out int lowerBound);

        [LibraryImport("oleaut32", EntryPoint = "SafeArrayGetUBound", SetLastError = false)]
        internal static partial int SafeArrayGetUBound(nint array, uint dimension, out int upperBound);
#else
        ...
        [DllImport("oleaut32", EntryPoint = "SafeArrayCreateVector", SetLastError = false, ExactSpelling = true)]
        internal static extern nint SafeArrayCreateVector(ushort variantType, int lowerBound, uint count);

        [DllImport("oleaut32", EntryPoint = "SafeArrayDestroy", SetLastError = false, ExactSpelling = true)]
        internal static extern int SafeArrayDestroy(nint array);

        [DllImport("oleaut32", EntryPoint = "SafeArrayAccessData", SetLastError = false, ExactSpelling = true)]
        internal static extern int SafeArrayAccessData(nint array, out nint data);

        [DllImport("oleaut32", EntryPoint = "SafeArrayUnaccessData", SetLastError = false, ExactSpelling = true)]
        internal static extern int SafeArrayUnaccessData(nint array);

        [DllImport("oleaut32", EntryPoint = "SafeArrayGetVartype", SetLastError = false, ExactSpelling = true)]
        internal static extern int SafeArrayGetVartype(nint array, out ushort variantType);

        [DllImport("oleaut32", EntryPoint = "SafeArrayGetLBound", SetLastError = false, ExactSpelling = true)]
        internal static extern int SafeArrayGetLBound(nint array, uint dimension, out int lowerBound);

        [DllImport("oleaut32", EntryPoint = "SafeArrayGetUBound", SetLastError = false, ExactSpelling = true)]
        internal static extern int SafeArrayGetUBound(nint array, uint dimension, out int upperBound);
#endif
    }
}
