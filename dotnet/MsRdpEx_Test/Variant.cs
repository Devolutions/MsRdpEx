...
namespace MsRdpEx.Tests
{
    public unsafe class VariantTests
    {
        ...
        [Fact]
        public void ConvertToManaged_NormalizesByReferenceValues()
        {
            short nativeBoolean = VariantBool.TrueValue;
            var boolean = new NativeVariant(VariantType.ByRefModifier | VariantType.Boolean)
            {
                Content1 = (nint)(&nativeBoolean)
            };
            Assert.IsType<bool>(VariantMarshaller.ConvertToManaged(boolean));
            Assert.True((bool)VariantMarshaller.ConvertToManaged(boolean)!);

            long nativeCurrency = decimal.ToOACurrency(12.34m);
            var currency = new NativeVariant(VariantType.ByRefModifier | VariantType.Currency)
            {
                Content1 = (nint)(&nativeCurrency)
            };
            Assert.Equal(12.34m, Assert.IsType<decimal>(VariantMarshaller.ConvertToManaged(currency)));

            double nativeDate = 45200.5d;
            var date = new NativeVariant(VariantType.ByRefModifier | VariantType.DateTime)
            {
                Content1 = (nint)(&nativeDate)
            };
            Assert.Equal(DateTime.FromOADate(nativeDate), Assert.IsType<DateTime>(VariantMarshaller.ConvertToManaged(date)));
        }

        [Fact]
        public void VariantMarshaller_RoundTripsByteArraySafeArray()
        {
            byte[] expected = [0x01, 0x02, 0x03, 0xFF];
            NativeVariant native = VariantMarshaller.ConvertToUnmanaged(expected);
            try
            {
                Assert.Equal(VariantType.ArrayModifier | VariantType.UInt8, native.Type);
                Assert.Equal(expected, Assert.IsType<byte[]>(VariantMarshaller.ConvertToManaged(native)));
            }
            finally
            {
                VariantMarshaller.Free(native);
            }
        }
