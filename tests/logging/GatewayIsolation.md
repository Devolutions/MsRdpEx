...
`logging.gateway-shutdown.*` verifies completion of production DLL teardown during
explicit unload and process exit, including when Windows terminates a worker holding
the gateway binding lock.
